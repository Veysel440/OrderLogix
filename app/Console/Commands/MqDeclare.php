...
class MqDeclare extends Command
{
    ...
    protected $description = 'Declare exchanges, queues, bindings for order flow';
    ...
    public function handle(): int
    {
        ...
        $c = \App\Services\Rabbit\ConnectionFactory::connect();
        $ch = $c->channel();
        ...
        $ch->queue_declare($ordersQ,false,true,false,false,false,new \PhpAmqpLib\Wire\AMQPTable([
            'x-queue-type'=>'quorum',
            'x-dead-letter-exchange'=>$ordersDlx,
        ]));
        $ch->queue_bind($ordersQ,$ordersEx,'order.placed');

        $ch->queue_declare($invQ,false,true,false,false,false,new \PhpAmqpLib\Wire\AMQPTable([
            'x-queue-type'=>'quorum',
            'x-dead-letter-exchange'=>$invDlx,
        ]));
        $ch->queue_bind($invQ,$invEx,'inventory.reserve');

        $ch->queue_declare($invDlq,false,true,false,false,false,new \PhpAmqpLib\Wire\AMQPTable([
            'x-queue-type'=>'quorum',
        ]));
        ...
        $ch->queue_declare($parkQ,false,true,false,false,false,new \PhpAmqpLib\Wire\AMQPTable([
            'x-queue-type'=>'quorum',
        ]));
        $ch->queue_bind($parkQ,$parkEx,'');

        $ch->close(); $c->close();
        $this->info('Topology updated.');
        return self::SUCCESS;
    }
}
