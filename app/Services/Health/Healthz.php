...
final class Healthz
{
    /** @return array{status:string, db:string, amqp:array<string,string>, mgmt:array<string,string>, queues:array<string,array<string,int|string>>, time:string} */
    public static function check(): array
    {
        ...
        foreach ($ctxs as $ctx) {
            ...
            $v = urlencode($vhost);
            $url = rtrim((string) env('RABBITMQ_MGMT_URL','http://127.0.0.1:15672'), '/')."/api/aliveness-test/{$v}";
            try {
            ...
            if (!$requireMgmt && $mgmt[$ctx] === 'err') { $mgmt[$ctx] = 'skip'; }
        }

        $queues = [];
        $requireQueues = filter_var(env('HEALTH_REQUIRE_QUEUES', true), FILTER_VALIDATE_BOOLEAN);
        $expected = array_values(array_filter(array_map('trim', explode(',', (string) env('HEALTH_EXPECT_QUEUES','')))));
        if ($expected) {
        ...
        $overall = ($okDb && $okAmqp && (($requireMgmt?$okMgmt:true)) && (($requireQueues?$okQueues:true))) ? 'ok' : 'degraded';
